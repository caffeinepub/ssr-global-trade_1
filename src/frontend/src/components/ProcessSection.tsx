const steps = [
  {
    ...
    description:
      "Responsibly sourced from trusted suppliers with full transparency and fair practices.",
  },
  {
    number: "02",
    title: "Sorting & Quality Inspection",
    description:
      "Rigorous quality checks at every stage — length, texture, strength, and authenticity verified.",
  },
  {
    number: "03",
    title: "Cleaning & Processing",
    description:
      "Advanced professional processing for premium quality, preserving natural texture and shine.",
  },
  {
    number: "04",
    title: "Secure Packaging",
    description:
      "Custom packaging with your branding, ensuring products arrive in perfect condition.",
  },
  {
    number: "05",
    title: "Worldwide Delivery",
    description:
      "Fast and secure international shipping via DHL / FedEx with full tracking.",
  },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="font-heading font-black text-cream mb-4 text-4xl sm:text-5xl">
            From Sourcing to{" "}
            <span className="gold-text-gradient">Your Door</span>
          </h2>
              ...
              {steps.map((step, index) => (
                ...
                >
                  ...
                  <div className="relative z-10 w-20 h-20 flex flex-col items-center justify-center bg-charcoal-900 border-2 border-gold/40 mb-6 group hover:border-gold transition-colors">
                    <span className="font-heading text-gold text-xl font-black">
                      {step.number}
                    </span>
                  </div>
                  <h3 className="font-heading font-bold text-cream text-base mb-2">
                    {step.title}
                  </h3>
                  <p className="text-cream-muted text-xs leading-relaxed font-medium">
                    {step.description}
                  </p>
                  {/* Arrow connector (not last) */}
                  {index < steps.length - 1 && (
                    <div className="absolute top-10 -right-2 z-20 text-gold/40 text-lg">
                      ›
                    </div>
                  )}
                </div>
            ...
            <div key={step.number} className="flex gap-6">
              ...
              <div className="flex flex-col items-center">
                <div className="w-14 h-14 flex-shrink-0 flex items-center justify-center border-2 border-gold/40 bg-charcoal-800">
                  <span className="font-heading text-gold text-lg font-black">
                    {step.number}
                  </span>
                </div>
                {index < steps.length - 1 && (
              ...
              <div className={`pb-8 ${index === steps.length - 1 ? "" : ""}`}>
                <h3 className="font-heading font-bold text-cream text-xl mb-2 mt-3">
                  {step.title}
                </h3>
